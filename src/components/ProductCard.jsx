...
const ProductCard = ({ product }) => {
  ...
  return (
    <motion.div
      ...
      onClick={handleClick}
    >
      {/* Logo toko pojok kiri atas */}
      <img
        src={logo}
        alt="Logo"
        className="absolute top-2 left-2 w-8 h-8 rounded-full bg-white p-1 shadow z-10"
      />

      {/* Gambar Produk */}
      <img
        src={product.gambar || 'https://via.placeholder.com/300x200?text=No+Image'}
        alt={product.nama}
      ...
      {/* Info Produk */}
      <div className="p-4">
        <h3 className="text-xl font-semibold text-gray-800 truncate">{product.nama}</h3>
        <p className="text-sm text-gray-500 capitalize mb-1">{product.kategori}</p>
        <p className="text-red-600 font-bold text-lg">{formatHarga(product.harga)}</p>
      </div>
    </motion.div>
  );
};
